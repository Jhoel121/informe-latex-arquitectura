...
\caption[]{Resultados de Encuesta a Conductores (continuación)} \\
\toprule
\textbf{Indicador} & \textbf{Porcentaje} \\
\midrule
\endhead
...
\caption[]{Resultados de Encuesta a Trabajadores y Administradores (continuación)} \\
\toprule
\textbf{Indicador} & \textbf{Porcentaje} \\
\midrule
\endhead
...
\begin{longtable}{p{2.8cm} p{2cm} p{2.5cm} p{6.5cm}}
...
\caption[]{Actores Principales del Sistema (continuación)} \\
\toprule
\textbf{Actor} & \textbf{Tipo} & \textbf{Nivel} & \textbf{Responsabilidades principales} \\
\midrule
\endhead
...
\textit{Nota.} El tipo ``Principal'' indica interacción directa y frecuente con el sistema. El nivel se refiere a la capa de la arquitectura jerárquica a la que el actor tiene acceso.
...
\caption[]{Actores Secundarios y Sistemas Externos (continuación)} \\
\toprule
\textbf{Actor o sistema} & \textbf{Función en el sistema} \\
\midrule
\endhead
...
\textit{Nota.} Los actores secundarios no interactúan con la interfaz de usuario; son sistemas o componentes que complementan la operación de la plataforma.

\subsection{Diagrama de Contexto}

El diagrama de contexto sitúa al sistema Smart-Park como bloque central. Alrededor de este bloque se representan los actores y sistemas externos, cada uno conectado mediante una flecha etiquetada con el tipo de dato que intercambia.
...
\end{figure}
\vspace{-5pt}
\textit{Nota.} Elaboración propia. (Sustituir caratula.png por diagrama\_contexto.png)

La siguiente tabla describe los flujos de información entre el sistema y cada actor externo:
...
\caption[]{Intercambio de Datos en el Diagrama de Contexto (continuación)} \\
\toprule
\textbf{Actor / Sistema externo} & \textbf{Dirección} & \textbf{Datos intercambiados} \\
\midrule
\endhead
...
\textit{Nota.} La dirección ``Bidireccional'' indica que el sistema envía y recibe datos del actor. ``Entrante'' indica que el actor solo envía datos hacia el sistema.
