...
\caption[]{Características Principales del Sistema (continuación)} \\
\toprule
\textbf{Característica} & \textbf{Descripción} \\
\midrule
\endhead
...
\textit{Nota.} PWA = Progressive Web Application. RBAC = Role-Based Access Control. JWT = JSON Web Token. WSS = WebSocket Secure. CAD = Computer-Aided Design.
...
\begin{longtable}{p{3cm} p{2.8cm} p{8cm}}
...
\caption[]{Componentes (Contenedores) del Sistema (continuación)} \\
\toprule
\textbf{Contenedor} & \textbf{Tecnología} & \textbf{Responsabilidad} \\
\midrule
\endhead
...
\textit{Nota.} SPA = Single Page Application. AMQP = Advanced Message Queuing Protocol. Cada contenedor se ejecuta como un proceso independiente o un servicio gestionado en la infraestructura de Railway.

\subsection{Diagrama de Infraestructura}

El diagrama de infraestructura muestra la distribución física de los componentes del sistema, sus tecnologías base y los protocolos de comunicación entre ellos.

\begin{figure}[H]
\caption{Diagrama de Infraestructura de Despliegue Unificado de Smart-Park}
\centering
\includegraphics[width=0.8\textwidth]{imagenes/caratula.png}
\end{figure}
\vspace{-5pt}
\textit{Nota.} Elaboración propia. (Sustituir caratula.png por diagrama\_infraestructura.png)
